...
exports.defaults = function () {
    ...
    return {
        ...
        ignoreErrors : false, //whether to call next with error if shit happens
...
exports.middleware = function recordRate(options) {
    ...
    return function recordRate(req, res, nextFn) {
        ...
        if(options.ignoreErrors) {
            next = function(error) { //just ignore error
                if(error) {
                    console.error('Express-rate failed with error %s \n', error);
                }

                //we need this flag when we ignore errors as otherwise it would call next twice and that would result in empty'ing route's middleware list
                //increment is two step process without any error checking in between so it results in next being called twice
                if(!nextCalled) {
                    nextFn();
                    nextCalled = true;
                }
